...
Keeping in mind Moltres proven capabilities, this work aimed for a heterogeneous calculation in a prismatic \gls{HTGR}.
...
Serpent obtained the group constants for three materials: moderator, coolant, and fuel compact.
...
The diffusion calculation had 1.84$\times 10^5$ \glspl{DoF}/energy-group.
Moltres input file set an eigenvalue convergence tolerance of 1$\times$10$^{-8}$, see equation \ref{eq:moltres-tol}.
The calculation used a two energy groups with the following structure [0, 0.625eV, 20MeV].
...
\begin{align}
   ...
   \intertext{where}
   & J = \mbox{current density} [n/cm^2/s] \notag \\
   & D = \mbox{diffusion coefficient} [cm] \notag \\
   & \phi = \mbox{neutron flux.} [n/cm^2/s] \notag
\end{align}
...
\begin{itemize}
	...
	\item The anisotropic energy-transfer due to scattering is negligible in group-to-group scattering.
\end{itemize}
...
These results suggest that a heterogeneous diffusion calculation of the prismatic fuel column challenges the limitations of the diffusion theory.
...
Serpent calculated the homogeneous group constants of the fuel assembly, see equation \ref{eq:diffusion}.
...
Based on these results and discussion, we carry out homogeneous calculations with Moltres in the following sections.

\begin{figure}[htbp!]
	\centering
    \subfloat[Serpent flux.]{
        \includegraphics[width=0.45\textwidth]{figures-neutronics/standard-column-detector-Axial}
    }
    \subfloat[Moltres flux.]{
