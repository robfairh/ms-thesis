...
Based on the previous section analyses, this analysis use the energy group structure $15d$ from Table \ref{tab:energygroups}.
...
Moltres model geometry and mesh, which had 3.0 $\times 10^5$ elements and 1.60 $\times 10^5$ nodes, using Gmsh.
...
To compute the fluxes, the simulations recorded the neutron flux in an axial and a radial flux detectors in arbitrary regions of the reactor, shown in Figure \ref{fig:fullcore-detectors}.
...
% This section's analysis remains qualitative.
% Serpent's axial flux is an average over radial direction, while Moltres' flux is the point-wise axial flux.
% A better comparison would use an axial detector for Serpent simulation with a very small volume centered on the fuel assembly's center.
% However, Monte Carlo simulations require of 
% Given the nature of the detectors, in a full core analysis, the 
% Serpent computes the average flux in the several bins that conform a detector.
% Moltres notion of a detector is the flux over a line.
...
To reduce the memory requirements, a Python script collapsed the group constants to a smaller number of energy groups (refer to Section \ref{appendix:group-const-condense} for further detail on group constants handling).
...
% OECD-Benchmark
Section \ref{sec:ph1e1} describes the results of the Phase I Exercise 1 of the OECD/NEA MHTGR-350 Benchmark using Moltres.
...
To circumvent this, the simulations approximated the periodic \gls{BC} with a reflective BC.
...
The combined effect of the approximation led to a large error in the control rod worth, while it had only a small influence on the axial offset.
