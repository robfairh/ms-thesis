...
This chapter comprehends the following sections: Section \ref{sec:neut-prelim} describes some preliminary studies that help set up Serpent and Moltres simulations, Section \ref{sec:neut-serpent} presents several studies comparing Moltres and Serpent results, and Section \ref{sec:ph1e1} discusses Moltres results of Phase I Exercise 1 of the Benchmark.
...
The study used a hexagonal unit cell model that included the fuel compact, a helium gap, and the surrounding graphite.
...
In a per assembly homogenization, the diffusion code models a global neutronic representation of the assembly (we will refer to diffusion calculations using this type as homogeneous calculations).
A pin-by-pin homogenization treats the pin or assembly heterogeneities, yielding a detailed neutronic representation of the fuel pins (we will refer to diffusion calculations of this type as heterogeneous calculations).

% Previous works using Moltres
Previous works \cite{lindsay_introduction_2018}\cite{pater_multiphysics_2019} have used Moltres, a heterogeneous solver, for the analysis of \glspl{MSR}.
...
Keeping in mind Moltres previous works, this worked aimed for a heterogeneous calculation in a prismatic \gls{HTGR}.
We modeled a fuel column of the MHTGR-350 and generated the group constants using Serpent.
...
Serpent ran 5$\times 10^5$ neutrons per cycle, 400 active cycles, and 100 inactive cycles for the calculations.
% The \gls{Keff} was 1.41933.
Taking advantage of the problem's symmetry, Moltres modeled only one-twelfth of the fuel column shown in Figure \ref{fig:fuelcolumn}.
...
Moltres input file set an eigenvalue convergence tolerance of 1$\times$10$^{-8}$.
The calculation used a two energy group structure with a thermal cutoff at 0.625eV.
...
Next, we conducted a feasibility study for the homogeneous calculation of the fuel assembly in Moltres.
...
Comparing Moltres results with Serpent results, Serpent's \gls{Keff} was 1.41933 while Moltres' was 1.4078788.
...
Serpent's flux is the average flux over the fuel column volume; Moltres flux is the point-wise flux over the $z$-axis.
...
This characteristic motivated the analysis of two cases: one fuel column that does not have \glspl{LBP} and one that does.
The LBPs' locations are the six corners of the fuel assembly, Figure \ref{fig:fuelassembly}.
...
For the LBP at 1200K case, the flux in Moltres is larger and more concave than Serpent flux.
...
Additionally, the radial flux in Serpent reveals the location of the LBPs in the fuel assemblies.
...
The radial fluxes were close in shape and magnitude; however, the radial flux in the diffusion calculation failed to capture the flux variation near the LBPs.
