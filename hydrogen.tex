...
To find the most efficient strategy, I not only consider HTGRs but also other type of reactors.
...
This section centers its focus on the transportation sector.
...
This section focuses on the electricity generation sector and the duck curve problem.
...
The prediction uses the linear regression that produces the worst-case scenario.
...
To obtain a prediction for 2050, the growth factor scales up the hourly data.
...
Finally, subtracting the solar production from the total load gives the net load or demand ($D_{NET}$).
...
The choice of an $\eta$ of 33$\%$, which yields a reactor power of 75.8 MWt.
For the \gls{HTE} case, the reactor's choice is an HTGR with an outlet temperature of 850$^{\circ}$C.
Considering an $\eta$ of 49.8$\%$, yields a reactor of 50.2 MWt.
...
I calculated the new peak using equation \ref{eq:newpeak}.
...
This thesis proposed a mitigation strategy that uses a microreactor of 25 MWe.
For such a reactor, this thesis quantified the mass of hydrogen produced by the different methods during the day.
