...
The second generation of nodal schemes derived spatial coupling relationships by applying the \gls{TIP}.
...
The first engineering application of \gls{FEM} was in the field of structural engineering and dated back to 1956.
...
Throughout the last four decades, many codes have used the \gls{FEM} to solve the diffusion equation.
...
The list of \gls{FEM} diffusion solvers is more extensive, but we focus on the best-documented codes in the open literature.
...
The code solves either the diffusion equation or the transport equation through the SN method.
...
% CAPP
In 2008, the \gls{KAERI} published an article \cite{lee_development_2008} that presented the code CAPP.
...
The article validated the code with two benchmark problems: first, the IAEA PWR benchmark problem; 
second, Phase I Exercise 1 of the OECD/NEA PBMR-400 Benchmark \cite{reitsma_oecd-neansc_2008}.
...
To validate their model, the authors solved a two-dimensional model of the PMR-200 at the \gls{BOC}.
...
They conducted several studies, such as a core depletion calculation, a core depletion calculation with a critical control rod position search, and the analysis of the bypass flow effects on the coupled calculations.
...
\gls{INL} has initially developed the Pronghorn code to model \glspl{PRMR}.
...
It was responsible for modeling the plant system layout, including the hot and cold ducts, the helium circulator, and the steam generator.
...
One conclusion of this work agrees with the previous bibliography \cite{gulf_oil_company_nuclear_1973} \cite{duderstadt_nuclear_1976} that the energy spectrum is critical to yield an accurate description of a nuclear reactor using a few groups.

ANL's study helps set up properly nodal diffusion calculations for an \gls{HTGR}.
...
In 2015, \gls{INL} presented their results \cite{strydom_results_2015} for an \gls{IAEA} \gls{CRP} \cite{tyobeka_htgr_2011} and showed that the homogenization of the compact material underestimates notably the eigenvalue.
...
Hence, to consider the interaction between the neutronics and thermal-hydraulics behavior, a coupled analysis is necessary \cite{tak_cappgamma_2016}.

% damian_vhtr_2008
In 2008, Damian et al. \cite{damian_vhtr_2008} conducted a study aimed at understanding the physical aspects of the annular core and the passive safety features of a prismatic \gls{HTGR}.
...
Such study used CAST3M \cite{studer_cast3marcturus_2007} code to solve the three-dimensional heat conduction in the solid coupled to the one-dimensional thermal-hydraulic equations in the coolant.
...
The studies included the variation of the helium by-pass fraction, average power density, core geometry, reflector materials, and fuel loading strategy.
One of their results exhibited that with the reduction of the bypass fraction the average reflector temperature rises.
Another result showed that using magnesium oxide as the reflector material yields lower temperatures for both normal operation and transients.
...
The main objective of the exercise was to validate the code and also identify technical challenges for future development.
The authors presented an interesting analysis in which they compared the results from the coupled simulation and the stand-alone simulations.
...
% tak_cappgamma_2016
Another article \cite{tak_cappgamma_2016} introduced a coupling between the CAPP code and the GAMMA+ code.
...
The code uses the one-dimensional form of the mass, momentum, energy, and species conservation equations to solve the flow and temperature distribution of the fluid.
...
Some of their most relevant results showed that the maximum fuel temperature reaches a peak near \gls{MOC}.
...
% Benchmarks Intro
The prismatic \gls{HTGR} tools available have lagged behind the state of the art compared to \glspl{LWR}.
...
In addition to the development of new methods, it is essential to define appropriate benchmarks to compare the capabilities of various tools.
...
The purpose of this benchmark exercise is to compare various coupled reactor physics and thermal-hydraulic analysis methods.
...
Phase II consists of the transient cases.
...
The objective of the \gls{CRP} was to determine the uncertainty in \gls{HTGR} calculations at all stages of coupled reactor physics/thermal-hydraulics and depletion calculations.
This \gls{CRP} is a natural continuation of the previous IAEA and OECD/NEA international activities \cite{iaea_evaluation_2003}\cite{reitsma_oecd-neansc_2008} on Verification and Validation of available capabilities for \gls{HTGR} simulations.
...
Phase II consists of design calculations, which are the coupled steady-state neutronics/thermal-hydraulics calculations with and without a depletion calculation.
