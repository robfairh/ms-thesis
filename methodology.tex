...
\begin{align}
   ...
   \intertext{where}
   & J^-_g(r, t) = \mbox{incoming current density } [n \cdot cm^{-2} \cdot s^{-1}] \notag \\
   & r_s = \mbox{position of the boundary } [cm] \notag \\
   & \hat{n_s} = \mbox{normal direction to the boundary } [-] \notag \\
   & J_g(r, t) = \mbox{current density } [n \cdot cm^{-2} \cdot s^{-1}] \notag \\
...
% Reactivity control
A combination of lumped burnable poison and control rods manage the core reactivity.
...
This thesis divides Moltres simulation results into Chapters \ref{ch:neutronics} and \ref{ch:thermalfluids}.
Chapter \ref{ch:neutronics} focuses on neutronics stand-alone simulations.
Chapter \ref{ch:thermalfluids} focuses on thermal-fluids stand-alone and neutronics/thermal-fluid simulations.
