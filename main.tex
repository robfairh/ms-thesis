...
These studies focus on local models such as the unit cell and the fuel column problems, for which Moltres results differ by less 2\% to the published results.
Further studies analyze the possibility of extending the thermal-fluids model implemented in this problems to a full-core simulation, finding a high memory requirement imposed by the simulations.
...
The study's results were within a 11.3\% difference to the published results, concluding that further analysis is required.

% results: ch5-coupled
Regarding coupled neutronics/thermal-fluids, the analysis describes Phase I Exercise 3 of the benchmark.
The exercise uses a simplified model that help to visualize some of the essential aspects of a multi-physics simulations in Moltres.
This exercise finds some flaws in Moltres model and sets a basis for future work.

% results: ch 6
This thesis aligns with the University of Illinois goals to reduce carbon emissions from the electricity and transportation sectors on its campus.
By introducing a nuclear reactor coupled to a hydrogen plant as a solution, this work focused on two main analysis.
The first analysis studied the conversion of the university fleet along with the mass transit transport system in Urbana-Champaign to Fuel Cell Electric Vehicles.
The second analysis studied the duck curve phenomenon in the university's grid and introduces a mitigation strategy that may reduce the reliance on dispatchable sources.
