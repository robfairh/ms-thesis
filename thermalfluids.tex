...
As the previous section's analysis has shown, the fuel-to-coolant temperature difference is small in the outlet for a sinusoidal power profile.
...
The model assigns a number to each coolant channel and the bypass-gap, as shown in Figure \ref{fig:th-val-assem-model-a}.
...
Table \ref{tab:th-val-assem-results} compares the results determined in this work using Moltres/MOOSE to Sato's results \cite{sato_computational_2010}.
...
The solution of Cases 3 and 4 depend on the temperature, requiring a second level of iterations that updates the material properties for the computed temperatures.
The convergence criteria were 1$^{\circ}$C for the maximum coolant and fuel temperatures.
A Python script external to Moltres holds these iterative solvers.
...
The radii of the rings preserve the assemblies' volume.
...
On the other hand, the maximum fuel temperature is sensitive to the fuel thickness, requiring conservation.
...
This chapter met the objective to predict the prismatic HTGRs temperature profile accurately; however, it achieved it partially.
