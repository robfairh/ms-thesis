...
Those simulations required running two separate input files, where the output of the global model served as input of the sub-channel model.
...
However, MOOSE framework does not count with the capability to couple this specific problem input files at the moment.
To solve exercise 3, we created a different model that homogenizes the fuel and moderator materials.
The solution did not differentiate between moderator and fuel requiring the simulation of only the global model.
...
Exercise 3a requires using the material properties from Exercise 2c.
This section used a simplified model that used the material properties from Exercise 2a, shown in Table \ref{tab:th-ex2a}.
The benchmark prescribes the group constants of 232 regions in the reactor.
Table \ref{tab:coupled-rz} shows what benchmark sub-domains integrate each model regions.
The simulations used a three energy-group structure.
Table \ref{tab:coupled-eg} what benchmark group numbers integrate each model group.
Conducting this exercise required the development of a tool to translate the benchmark group constants to Moltres format.
The tool was responsible for homogenizing and collapsing the cross sections as well.
The problem assumed the same boundary conditions from Section \ref{sec:ph1ex2}.

Moltres has the capability to decouple the neutronics from the thermal-fluids effects.
For the sake of comparison, we conducted the exercise with and without thermal feedback.
The calculations without thermal feedback assumed the material group constants at 550 $^{\circ}$C.
...
Consequently, the heat production shifts towards the reactor top and the temperatures near the reactor outlet decrease.
The radial flux does not change considerably, hence the temperature profile does not change considerably either.
The whole radial temperature profile moves up because of the power shit towards the top.
...
Section \ref{sec:ph1ex3} used a heterogeneous thermal-fluids model to solve the temperature in the reactor.
The problem of using a heterogeneous model is that each node holds the value of the temperature in that particular material.
A coolant node holds the information of the coolant temperature and computes the thermal feedback with such information instead of the moderator and fuel temperature.
...
Section \ref{sec:ph1ex3} thermal-fluids model homogenized the fuel and the moderator into one material.
Consequently, the model does not differentiate the fuel from the moderator temperature.
Such homogenization assumes that both moderator and fuel are in thermal equilibrium and hence have the same temperature.
However, a coupling model is unable to correctly calculate the thermal feedback if it does not take into account the difference between the moderator and the fuel temperatures \cite{damian_vhtr_2008}.

We denominate this issue as the 'Homogenization dilemma.'
To properly compute neutronics, the diffusion solver must use homogenized parameters.
On the other hand, to properly compute the thermal feedback, the mesh node requires the moderator and fuel temperatures which requires a heterogeneous calculation in the thermal-fluids model.

% Wrap-up
A heterogeneous calculation is valid, but the thermal feedback should use fuel and moderator average temperatures instead.
Another approach would be to homogenize the materials in the fuel assembly and calculating an homogeneous temperature for each material.
...
However, the use of such model in the open literature is not always correct as many articles do not differentiate between moderator and fuel temperatures.
...
Although the model was very simple, it allowed to visualize some of the basic characteristics of a prismatic HTGR multi-physic simulation in Moltres.
This exercise lead to Section \ref{sec:discuss} which described some of the flaws found in the model.
This exercise also sets the basis for future work that Section \ref{ref:futwork} will introduce.