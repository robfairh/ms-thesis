...
% % explanation of what the model needs
% Moltres is a heterogeneous solver.
% In a heterogenous solver, each node of the mesh holds the information of each variable.
% In a diffusion simulation, the homogenization should occur for all the variables.
% The thermal-fluids solver should obtain the moderator and the fuel temperature in the assembly.
% A heterogenous strategy solves the thermal-fluids in an HTGR, however, each node in the mesh holds the information of the temperature in that node only.
...
% In order for the simulation to be accurate, the heterogeneous temperature solver should average the temprature in the assembly and provide that information to the diffusion solver.

% As Damian \cite{damian_vhtr_2008} mentioned, a coupling model is unable to correctly calculate the thermal feedback if it does not take into account the difference between the moderator and the fissile isotopes temperatures.
% de-homogenization calculation which guarantees a better description of the local thermal calculation in the fuel
