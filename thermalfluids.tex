...
Future works will focus on adding the capability to model such heat transfer mechanism.

To circumvent this barrier, we developed a second model based on Stainsby's approach \cite{stainsby_investigation_2008}.
The model uses a global model to obtain the coolant temperatures in the core and then it uses a sub-channel approach to obtain the fuel and moderator average temperatures.
Figure \ref{fig:ex2a-2nd-model} displays the model geometry.
The model uses three rings to define the core solid structures and other three for the coolant flow in the core.
We calculated the rings radii by preserving the assemblies volume.
The inner and outer rings represent the inner and outer reflectors.
The middle ring defines the fuel rings, which are three (see Figure \cite{oecd_nea_benchmark_2017}).
The coolant ring thicknesses preserve the coolant volume in each of the fuel rings.

The model uses the material properties from Table \ref{tab:th-ex2a}.

Figure \ref{fig:ex2a-temps} presents the coolant and solid temperatures in the different rings.
...
\begin{figure}[htbp!]
  \centering
  \includegraphics[width=0.25\linewidth]{figures-thermal/ex2a-meshD2}
  \hfill
...
\begin{figure}[htbp!]
  \centering
  \includegraphics[width=0.25\linewidth]{figures-thermal/ex2a-across}
  \hfill
  \caption{Model geometry.}
...
\begin{figure}[htbp!]
  \centering
  \includegraphics[width=0.25\linewidth]{figures-thermal/ex2a-meshC2}
  \hfill
  \caption{Model geometry.}
