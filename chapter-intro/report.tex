...
% Advantages of HTGRs, TRISO particles, 
...
The fuel particle is a microsphere of about 0.8 mm diameter.
It includes a fuel kernel surrounded by a porous carbon layer (or buffer), followed successively by an \gls{IPyC} layer, \gls{SiC} layer, and an \gls{OPyC} layer.

% Explantion of the layer functions
...
% The \gls{IPyC} layer protects the kernel from chloride during \gls{SiC} decomposition.
% It also contributes to fission gas retention \cite{demkowicz_triso_2019}.
...
% The \gls{OPyC} layer contributes to fission gas retention and protecst \gls{SiC} layer during handling.
...
The approach to safety relies primarily on the natural processes of thermal radiation, conductio
...
The modular HTGR adds the unique characteristic of being able to cool the reactor entirely by
passive heat transfer mechanisms following postulated accidents without exceeding the failure
temperature of the coated particles. This characteristic has been achieved by deliberately
decreasing the core power level and configuring the reactor so that natural heat removal
processes can limit fuel temperatures to levels at which the release of fission products from
the reactor system to the environment is insignificant for postulated accidents. Even for
extreme accidents having very low probabilities of occurrence, the cumulative fission product
release at the site boundary is estimated to be below those acceptable under defined protective
action guidelines [2].
\cite{brey_development_2001}


History of the MHTGR 
\cite{brey_development_2001}



\glspl{PMR} use uranium fuel, graphite as the moderator, and helium as coolant
\cite{breeze_nuclear_2014}.
...
\section{MHTGR-350 Reactor Summary}
